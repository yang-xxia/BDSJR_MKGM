...
tb_writer = SummaryWriter('./logs/' + timestr+'rcnnmamba2')
...
class Engine(object):
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, optimizer=None):
        ...
        for epoch in range(self.state['start_epoch'], self.state['max_epochs']):
            ...
            tb_writer.add_scalar('train/map', prec1.item(), epoch)
            # remember best prec@1 and save checkpoint
            is_best = prec1 > self.state['best_score']
            self.state['best_score'] = max(prec1, self.state['best_score'])
            tb_writer.add_scalar('train/best', self.state['best_score'], epoch)
...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        FMR0 = self.state['ap_meter'].calculate_average_accuracy()
        FMR1 = self.state['ap_meter'].calculate_one_error_accuracy()
        FMR2 = self.state['ap_meter'].calculate_at_most_two_errors_accuracy()
        FMR = self.state['ap_meter'].calculate_full_accuracy()
        ...
        if display:
            if training:
                ...
                print('OP: {OP:.4f}\t'
                      ...
                      'FMR1: {FMR1:.4f}\t'
                      'FMR2: {FMR2:.4f}\t'
                      'FMR: {FMR:.4f}\t'.format(OP=OP, OR=OR, OF1=OF1, CP=CP, CR=CR, CF1=CF1, FMR0=FMR0, FMR1=FMR1, FMR2=FMR2, FMR=FMR))
                print('FMR0: {FMR0:.4f}\t'
                      'FMR1: {FMR1:.4f}\t'
                      'FMR2: {FMR2:.4f}\t'
                      'FMR: {FMR:.4f}\t'.format(FMR0=FMR0, FMR1=FMR1, FMR2=FMR2, FMR=FMR))
            else:
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        pool = torch.autograd.Variable(self.state['input']).float()
        ...
        # compute output
        self.state['output'] = model(feature_var, pool)
        self.state['loss'] = criterion(self.state['output'], target_var)
